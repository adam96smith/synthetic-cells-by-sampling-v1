...
class DownConv(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size=3, pooling=True, planar=False, activation='relu',
        ...
        self.conv1 = conv3(
            self.in_channels, self.out_channels, kernel_size=self.kernel_size, planar=planar, dim=dim, padding=padding
        )
        self.conv2 = conv3(
            self.out_channels, self.out_channels, kernel_size=self.kernel_size, planar=planar, dim=dim, padding=padding
        )
...
class UpConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
        ...
        if self.merge_mode == 'concat':
            self.conv1 = conv3(
                2*self.out_channels, self.out_channels, kernel_size=self.kernel_size, planar=planar, dim=dim, padding=padding
            )
        else:
...
class UNet(nn.Module):
    ...
    Args:
        ...
        out_channels: Number of output channels (in classification/semantic
            ...
            Default: 2
        kernel_size: Size of kernel in conv layers. Default=3
            Default: 2
