...
from my_elektronn3.data.transforms import random_blurring
from my_elektronn3.data.transforms.random import Normal, HalfNormal, RandInt
...
def circleKern(N, dim = 3): # circle/spherical kernel to apply dilate/erode/smoothing
...
def distmap_optimized(mask, size=5, d_stop=20): # optimised approximation for edt near target edge, only supports 3D as of 04/12/24
    """
    Optimized approximation of the distance map.
    Args:
        mask (torch.Tensor or np.ndarray): Binary 3D mask.
        size (int): Size of kernel for erosion/dilation.
        d_stop (int): Distance threshold for stopping.
    Returns:
        torch.Tensor or np.ndarray: Distance map approximation, same device/type as input.
    """
    # Convert input to PyTorch tensor if it's a numpy array
    if isinstance(mask, np.ndarray):
        mask_torch = torch.tensor(mask, dtype=torch.float32)
    else:
        mask_torch = mask.float()
    
    device = mask_torch.device  # Automatically detect the device of the input tensor
    mask_torch = mask_torch.to(device)
    output = torch.zeros_like(mask_torch, device=device)
    
    # Precompute kernel and convert to PyTorch
    kernel = circleKern(size, dim=3)
    kernel_torch = torch.tensor(kernel, dtype=torch.float32, device=device).unsqueeze(0).unsqueeze(0)

    # Erosion pass
    erode_shape = mask_torch.clone()
    output += 1 - erode_shape
    C1 = 0
    while erode_shape.sum() > 0 and C1 < d_stop:
        erode_shape = F.conv3d(
            erode_shape.unsqueeze(0).unsqueeze(0), 
            kernel_torch, 
            padding=size // 2
        ).squeeze(0).squeeze(0) > 0.5
        erode_shape = erode_shape.float()
        output += 1 - erode_shape
        C1 += (size - 1) / 2

    # Dilation pass
    dilate_shape = 1 - mask_torch.clone()
    output -= 1 - dilate_shape
    C2 = 0
    while dilate_shape.min() == 0 and C2 < d_stop:
        dilate_shape = F.conv3d(
            dilate_shape.unsqueeze(0).unsqueeze(0), 
            kernel_torch, 
            padding=size // 2
        ).squeeze(0).squeeze(0) > 0.5
        dilate_shape = dilate_shape.float()
        output -= 1 - dilate_shape
        C2 += (size - 1) / 2

    # Return output in the same format as the input
    if isinstance(mask, np.ndarray):
...
class DropIfTooMuchBG:
    ...
    def __call__(
        ...
        if np.sum(target == self.bg_id) / target.size > self.threshold:
            raise _DropSample
        return inp, targets  # Return inp, target unmodified
...
class Normalize:
    ...
    Args:
        ...
        p: Random scaling parameter for mean/std. 
            If nonzeros, mean/std +- p*mean/p*std
            If mean ~0 (zero_mean) then mean +- p*(std/2)
    """
    def __init__(
            self,
    ...
    def __call__(
            self,
            inp: np.ndarray,
            targets: Optional[List[np.ndarray]] = None  # returned without modifications
    ) -> Tuple[np.ndarray, np.ndarray]:
        if self.inplace:
        ...
        for c in channels:
            r_mean = 1 - (2*(np.random.rand()-0.5) * self.p)
            r_std = 1 - (2*(np.random.rand()-0.5) * self.p)
            if self.zero_mean: # mean is 0 so use std. to scale values
                normalized[c] = (inp[c] - r_mean*(.5*self.std[c])) / (r_std*self.std[c])
            else:
                normalized[c] = (inp[c] - r_mean*self.mean[c]) / (r_std*self.std[c])
...
class NormalizeMinMax:
    """
    Normalizes inputs with supplied per-channel based on min and max values. 
    Scales the fluorescent intensities to [0,1] - effective if the upper and lower bound is known!

    Args:
        min_val: Global mean value(s) of the inputs. Can either be a sequence
            of float values where each value corresponds to a channel
            or a single float value (only for single-channel data).
        max_val: Global standard deviation value(s) of the inputs. Can either
            be a sequence of float values where each value corresponds to a
            channel or a single float value (only for single-channel data).
        inplace: Apply in-place (works faster, needs less memory but overwrites
            inputs).
    ...
    def __init__(
            self,
            min_val: Optional[Union[Sequence[float], float]] = None,
            max_val: Optional[Union[Sequence[float], float]] = None,
            inplace: bool = False,
            channels: Optional[Sequence[int]] = None
    ):
        self.min_val = []
        ...
        if max_val is None:
            ...
            if isinstance(max_val, list):
                for v in max_val:
                    (self.max_val).append(v)
            else:
                (self.max_val).append(max_val)

        self.min_val = np.array(self.min_val)
        self.max_val = np.array(self.max_val)
        self.inplace = inplace
        self.channels = channels
    ...
    def __call__(
        ...
        if self.inplace:
            ...
            normalized = inp.copy()
        channels = range(inp.shape[0]) if self.channels is None else self.channels
        if not len(channels) == self.min_val.shape[0] == self.max_val.shape[0]:
            raise ValueError(f'min_val ({self.min_val.shape[0]}) and max_val ({self.max_val.shape[0]}) must have the same length as the C '
                             f'axis (number of channels) of the input ({inp.shape[0]}).')
            
        for c in channels: # apply normalisation
            # if None, auto
            if self.min_val[c] is None:
                minV = np.min(inp[c])
            else:
                minV = self.min_val[c]

            if self.max_val[c] is None:
                maxV = np.max(inp[c])
            else:
                maxV = self.max_val[c]
                
            normalized[c] = (inp[c] - minV) / (maxV - minV)
...
class RandomGaussianBlur:
    ...
    def __init__(
        ...
        if aniso_factor is None or aniso_factor == 1:
            ...
            aniso_factor = np.array([aniso_factor, 1, 1]) # Changed by AS 03/02/25
...
class AdditiveGaussianNoise:
    ...
    def __call__(
            self,
            inp: np.ndarray,
            targets: Optional[list] = []  # returned without modifications
            # TODO: fast in-place version
...
def _draw_debug_grid(
    ...
    if targets is not None and targets[0].ndim == inp.ndim - 1:
        for target in targets:
            target[::s] = v
    if inp.ndim == 4:
        inp[:, ::s] = v
...
class ElasticTransform:
    ...
    def __init__(
        ...
        self.channels = channels
        self.prob = prob
        self.target_discrete_ix = target_discrete_ix # ignored for the modifications
        self.aniso_factor = aniso_factor
        self.draw_debug_grid = draw_debug_grid
        self.step = step

        self.distmap_size = distmap_size
        self.distmap_stop = distmap_stop
    def __call__(
            self,
            inp: np.ndarray,
            targets: Optional[List[np.ndarray]] = None
        ...
        if inp.ndim == 4:
            if self.draw_debug_grid:
                inp, targets = _draw_debug_grid(inp, targets)
            ...
            if targets is not None:
                tsh = np.array(targets[0].shape[-3:])
            if targets is not None and np.any(ish != tsh):
                if self.draw_debug_grid:
                    inp, targets = _draw_debug_grid(inp, targets)
            ...
            # If there is a target, apply the same deformation field to the target
            if targets is not None:
                tsh = np.array(targets[0].shape[-2:])
            if targets is not None and np.any(ish != tsh):
                # Crop input re-indexing arrays to the target region and transform coordinates
                #  to the target' own frame by subtracting the input-target offset
                lo = (ish - tsh) // 2
        ...
        if targets is None:
            return deformed_img, targets
        else:
            deformed_targets = []
            for target in [targets[0]]: # only apply to target
            ...
            if len(targets) > 1: # only 1 or 2 
                lsr = distmap_optimized(deformed_targets[0][0], size=self.distmap_size, d_stop=self.distmap_stop)                
                lsr = np.reshape(lsr, (1,)+lsr.shape)
                deformed_targets.append(lsr)
                
            return deformed_img, deformed_targets
...
# Not Compatible with Patch: must be performed outside 
class IncreaseZDim:
    '''
    Transform for 3D input to be used outside Patch Sampler. ( end of data.cnndata.PatchCreator.warp_cut() )
    
    Uses interpolation on patch in z-dimension to artificially reduce anisotropy factor by a scale of 'sf'.
    CPU & GPU compatible.    
    '''
    def __init__(self,
                 sf: int = 2,
                 device: str = 'cpu', # not sure what to put here?
                 iterations: int = 3,
    ):        
        self.sf = sf # scale factor to increase z resolution
        self.device = device # cpu or gpu for speed up!
        self.iterations = iterations # gap-filling parameter for binary interpolation
    ...
    def __call__(self, inp, targets):
        ...
        if self.sf == 1:
            ...
            new_inp = self.zoom_optim(inp, order=3)  # increase the z-resolution
    
            new_targets = []
            for target in targets:
                
                 # zoom might cause gaps in binary array. Use dilation and erosion to fill
                if target.dtype == bool or target.dtype==int:
                    new_target = self.zoom_optim(target, order=0)
                    new_target = binary_dilation(new_target, iterations=self.iterations)
                    new_target = binary_erosion(new_target, iterations=self.iterations)
                else:
                    new_target = self.zoom_optim(target, order=3)
    
                new_targets.append(new_target)
    
            return new_inp, new_targets
            
    def zoom_optim(self, arr, order=3):
        """
        Optimized zoom function for 3D arrays. Uses GPU acceleration with PyTorch if self.device='cuda',
        otherwise falls back to scipy.ndimage.zoom on CPU.
        
        Parameters:
            arr (np.ndarray): Input 4D array with shape (C, D, H, W) or (1, D, H, W).
            order (int): Interpolation order (supports 0 or 3 for GPU; any valid order for CPU).
        
        Returns:
            np.ndarray: Zoomed array with the same shape except for the depth dimension.
        """
        assert arr.ndim == 4, "Input array must have 4 dimensions: (C, D, H, W)"
        if self.device == torch.device('cuda'):  # Use PyTorch for GPU-based computation
            assert order in [0, 3], "For GPU, only nearest (0) or cubic (3) interpolation is supported."
            
            # Convert input array to PyTorch tensor and move to the specified device
            tensor = torch.tensor(arr, dtype=torch.float32, device=self.device)
            
            # Determine interpolation mode
            mode = 'trilinear' if order == 3 else 'nearest'
            
            if tensor.shape[0] == 1:  # Single-channel optimization
                tensor = tensor.squeeze(0)  # Remove the first dimension
                zoomed_tensor = F.interpolate(
                    tensor.unsqueeze(0).unsqueeze(0),  # Add batch and channel dimensions
                    scale_factor=(self.sf, 1, 1),
                    mode=mode,
                    align_corners=(order == 3) if order==3 else None # Only set align_corners for trilinear
                ).squeeze(0).squeeze(0)  # Remove added dimensions
                zoomed_array = zoomed_tensor.cpu().numpy()[np.newaxis]  # Add back the channel dimension
            else:
                zoomed_tensor = F.interpolate(
                    tensor,
                    scale_factor=(self.sf, 1, 1),
                ...
                zoomed_array = zoomed_tensor.cpu().numpy()
            
            return zoomed_array
    
        else:  # Use scipy.ndimage.zoom for CPU-based computation
            if arr.shape[0] == 1:  # Single-channel optimization
                zoomed_array = zoom(arr[0], (self.sf, 1, 1), order=order)  # Increase the z-resolution
                zoomed_array = zoomed_array[np.newaxis]
            else:
                zoomed_array = np.zeros([arr.shape[0], int(self.sf * arr.shape[1]), arr.shape[2], arr.shape[3]])
                for i in range(arr.shape[0]):
                    zoomed_array[i] = zoom(arr[i], (sf, 1, 1), order=order)  # Increase the z-resolution
...
class SqueezeTarget:
    ...
    def __call__(
        ...
        if targets is None:
            return inp, targets
        return inp, [target.squeeze(axis=self.dim) for target in targets]
