...
from my_elektronn3.data import coord_transforms, transforms
from my_elektronn3.data.sources import DataSource, HDF5DataSource, slice_3d
...
class PatchCreator(data.Dataset):
    ...
    def __init__(
            ...
            target_sources: Optional[List[List[Tuple[str, str]]]] = None,
        ...
        if target_sources is not None:
            if not isinstance(target_sources[0], list):
                raise ValueError(
                'If target_sources is not None, target_sources must be a list of lists.'
                'eg. [[target],..] or [[target_1, target_2,...], ...]'
            )
        if not train:
            if warp_prob > 0:
    ...
    def _get_random_sample(self) -> Dict[str, Any]:
        input_src, target_srcs, i = self._getcube()  # get cube randomly
        warp_prob = self.warp_prob
        while True:
            try:
                inp, targets = self.warp_cut(input_src, target_srcs, warp_prob, self.warp_kwargs)
                if targets is not None:
                    for i in range(len(targets)):
                        targets[i] = targets[i].astype(self._target_dtype)
            ...
            try:
                inp, targets = self.transform(inp, targets)
        ...
        if targets is not None:
            sample['targets'] = [torch.as_tensor(target) for target in targets]
    ...
    def warp_cut(
            self,
            inp_src: DataSource,
            target_srcs: Optional[List[DataSource]],
        ...
        if target_srcs is None:
            target_src_shape = None
            target_patch_shape = None
        else:
            target_src_shape = target_srcs[0].shape # AMS24
        ...
        inp, targets = coord_transforms.warp_slice(
            inp_src=inp_src,
            patch_shape=self.patch_shape,
            M=M,
        ...
        for filt in self.filters:
            inp, targets = filt(inp, targets)
        
        if self.z_scale > 1: # interpolate the data in z-direction (save doing this outside script)
            z_interpolation = transforms.IncreaseZDim(sf=self.z_scale, device=self.device)
            inp, targets = z_interpolation(inp, targets)
    ...
    def _getcube(self) -> Tuple[DataSource, DataSource, int]:
        ...
        # print(self.cube_prios, i, len(self.inputs))
    ...
    def load_data(self) -> None:
        ...
        if self.cube_prios is None:
            ...
            if self.targets is None:
                ...
                self.cube_prios = [target.size for target in self.targets[0]] # AMS24
    ...
    def check_files(self) -> None:
        ...
        if self.target_sources is not None:
            for target_list in self.target_sources: # targets are now a list of list (of tuples) AMS24
                fullpaths.extend([f for f, _ in target_list])
        for p in fullpaths:
    ...
    def open_files(self) -> Tuple[List[DataSource], Optional[List[DataSource]]]:
        ...
        if self.target_sources is None:
            ...
            for (inp_fname, inp_key), target_data_list, cube_meta in zip(self.input_sources, self.target_sources, self.cube_meta):
                inp_source = HDF5DataSource(fname=inp_fname, key=inp_key, in_memory=self.in_memory)
                logger.info(f'  input:       {inp_fname}[{inp_key}]: {inp_source.shape} ({inp_source.dtype})')

                target_data = []
                for (target_fname, target_key) in target_data_list:
                    ...
                    target_data.append(target_source)
                        
                if not np.all(cube_meta == np.inf): # unsure on this
                    logger.info(f'  cube_meta:   {cube_meta}')
                    
                inp_sources.append(inp_source)
                target_sources.append(target_data) # list of lists to match new target format AMS24
        logger.info('')
