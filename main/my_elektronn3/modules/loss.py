...
class CombinedLoss(torch.nn.Module):
    ...
    def forward(self, *args):
        ...
        for crit, weight in zip(self.criteria, self.weight):
            if isinstance(crit, nn.CrossEntropyLoss): # need to unpack the list of targets to input tensor
                try:
                    dout, dtargets, dtarget_class = args
                    loss += weight * crit(dout, dtargets[0], dtarget_class)
                except:
                    dout, dtargets = args
                    loss += weight * crit(dout, dtargets[0])
            else:
...
class DynamicWeightedLoss(nn.Module):
    """Defines a dynamically weighted loss function that adjusts weights incrementally after each step.

    Args:
        criteria: List of loss criterion modules to be combined.
        weight: Initial weight assigned to each loss criterion (in the same order as `criteria`).
        weight_increment: Increment value or list of increments for updating weights dynamically.
            If a single float, all weights are adjusted by this value. If a list, it must match the length of `criteria`.
        min_weight: Minimum threshold for each weight.
        max_weight: Maximum threshold for each weight.
        device: The device on which the loss should be computed.
    """
    def __init__(
            self,
            criteria: Sequence[torch.nn.Module],
            weight: Optional[Sequence[float]] = None,
            weight_increment: Optional[Sequence[float]] = None,
        ...
        # Initialize weights
        if weight is None:
            self.weight = torch.ones(len(criteria), device=device) * 0.5
        else:
            self.weight = torch.tensor(weight, dtype=torch.float32, device=device)
            assert self.weight.shape == (len(criteria),)
        
        # Initialize weight increments
        if weight_increment is None:
            self.weight_increment = torch.zeros(len(criteria), device=device)
        else:
            if isinstance(weight_increment, (float, int)):
                self.weight_increment = torch.full((len(criteria),), weight_increment, device=device)
            else:
                self.weight_increment = torch.tensor(weight_increment, dtype=torch.float32, device=device)
                assert self.weight_increment.shape == (len(criteria),)
    
    def forward(self, *args):
        loss = torch.tensor(0., device=self.device)
        for crit, weight in zip(self.criteria, self.weight):
            if isinstance(crit, nn.CrossEntropyLoss):  # need to unpack for CrossEntropyLoss
                try:
                    dout, dtargets, dtarget_class = args
                    loss += weight * crit(dout, dtargets[0], dtarget_class)
                except ValueError:
    ...
    def update_weights(self):
        """Adjust the weights based on `weight_increment`, ensuring they stay within [min_weight, max_weight]."""
        self.weight += self.weight_increment
        self.weight = torch.clamp(self.weight, min=self.min_weight, max=self.max_weight)

        print('Updated Weights:', [float(i) for i in self.weight])


class BoundaryLoss(torch.nn.Module):
    """Multi-Channel Boundary Loss, inspired by method described in https://arxiv.org/abs/1812.07032

    Works for n-dimensional data. 
    Input:
    ``output`` - tensor of model output. 
    ``targets`` - list of tensors to be used to calculate BL. 
        targets[0] - target segmentation with n channels.
        targets[1] or [1:n] - level-set representation (lsr or phi from the paper)
                              for target channel n-1 (background left out)

    eg. 
    - 2-channel (background and foreground/cell) has 1x target segmentation, and 1x lsr data
    - 3-channel (background, cell 1 and cell 2) has 1x target segmentation and 2x lsr data for each cell.

    Note: Calculation is channel invariant, meaning the order of the lsr data does not matter!
    ...
    def forward(self, output, targets):
        ...
        # Extract level-set representations (LSR)
        lsr_list = torch.stack(targets[1:])
        
        # Compute the boundary term
        tmp = probs[:, 1:] * lsr_list
        
        # Compute the scale (normalize over negative distances only)
        scale = lsr_list[lsr_list < 0].abs().sum()
        
        # Avoid division by zero and ensure loss stays non-negative
        if scale > 0:
            loss = 1 - tmp.sum() / scale
            return torch.clamp(loss, min=0)
        else:
            loss = 0  # No labels in the image
            return loss

        # # Clamp loss to ensure it is non-negative
        # return torch.clamp(loss, min=0)


class CurvatureWeightedLoss(torch.nn.Module):
    def __init__(self, weights):
        super(CurvatureWeightedLoss, self).__init__()
    ...
    def forward(self, logits, targets):
        """
        logits: (N, C, D, H, W) - Model outputs (before softmax), with C=2 (background, foreground)
        targets: (N, D, H, W) - Ground truth segmentation labels (0 for background, 1 for foreground)
        weight_map: (N, D, H, W) - Per-pixel weight map
        """
        assert len(targets)==2
        ground_truth, curvature_seg = targets

        # calculate weighted map
        w0 = self.weights[0] * (ground_truth == 0)  # Background weight
        w1 = self.weights[1] * ((ground_truth == 1) * (curvature_seg == 0))  # Foreground non-curved weight
        w2 = self.weights[2] * ((ground_truth == 1) * (curvature_seg == 1))  # Foreground curved weight

        weight_map = w0 + w1 + w2

        ce_loss = self.criterion(logits, ground_truth)  # (N, D, H, W)
        weighted_loss = ce_loss * weight_map  # Apply weights
        
        return weighted_loss.mean()  # Return both for inspection


class MixedMaskedLoss(nn.Module):
    def __init__(self, p: float):
        """
        Mixed loss combining standard cross-entropy and weighted cross-entropy from curvature.
        """
        super(MixedMaskedLoss, self).__init__()
        self.criterion = nn.CrossEntropyLoss(reduction='none')  # Per-pixel loss
        self.weighting = CurvatureWeighting

    def forward(self, predictions, targets, weights):
        """
        Compute the mixed masked loss.

        Args:
            predictions (torch.Tensor): Model predictions (logits) with shape (N, C, D, H, W).
            targets (torch.Tensor): Ground truth labels with shape (N, D, H, W).
            curvature_weights (torch.Tensor): Weight map with shape (N, D, H, W).
        ...
        if isinstance(self.criterion, nn.CrossEntropyLoss()):
            loss_all = self.criterion(predictions, targets[0])  # Shape: (N, D, H, W)
        else:
            loss_all = self.criterion(predictions, targets)

        # Apply curvature-based weighting
        weighted_loss = loss_all * weights
...
class DiceLoss(torch.nn.Module):
    ...
    def forward(self, output, targets):
        probs = self.softmax(output)
        return self.dice(probs, targets[0], weight=self.weight, smooth=self.smooth)
