...
from my_elektronn3.training import handlers
from my_elektronn3.training.swa import SWA
from my_elektronn3.training.train_utils import pretty_string_time, create_preview_batch_from_knossos
from my_elektronn3.training.train_utils import Timer, HistoryTracker

from torch.utils import collect_env
from my_elektronn3.inference import Predictor
from my_elektronn3 import __file__ as arch_src
...
class Trainer:
    ...
    Args:
        ...
        save_full: Toggle to save minimum output. If true, only model.pt, the
                   current model is saved (same as final, no state_dicts saved
                   reducing the size out outputs drastically!).
    ...
    def __init__(
        ...
        self.scaler = torch.GradScaler(self.device.type, enabled=self.mixed_precision)
        ...
        self.step = 0
        self.epoch = 0
        self.loss_weight_update = False ## Default - True if you want to update weighted loss.
        self.loss_weight_delay = 0 ## non-zero if delay the update to loss function weights!
    ...
    def run(self, max_steps: int = 1, max_runtime=3600 * 24 * 7, plot_history=True) -> None:
        ...
        while not self.terminate:
            try:
                ...
                # if succesful training step and loss weight update activated, update loss function
                if self.loss_weight_update:
                    if self.step >= self.loss_weight_delay:
                        self.criterion.update_weights()
                    else:
                        print(f'Loss weight update delayed until step: {self.loss_weight_delay}, current step: {self.step}')
    ...
    def _train_step(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if targets is not None:
            dtarget = [target.to(self.device, non_blocking=True) for target in targets]
        else:
            dtargets = None
        dtarget_class = target_class.to(self.device, non_blocking=True) if target_class is not None else None
            
        # forward pass
        with amp.autocast(self.device.type, enabled=self.mixed_precision):
            dout = self.model(dinp)
            # print('Trainer Inp. Shape:', dinp.shape)
            # for i, tmp_dtarg in enumerate(dtarget):
            #     print(f'Trainer Targ. {i+1} Shape:', tmp_dtarg.shape)
            # print('Trainer Outp. Shape:', dout.shape)
            # print('Target Class:', dtarget_class)
    ...
    def _train(self, max_steps, max_runtime):
        ...
        for i, batch in enumerate(batch_iter):
            ...
            with torch.no_grad():
                loss = float(dloss)
                targets = batch.get('targets')
                mean_target = float(targets[0].to(torch.float32).mean()) if targets is not None else 0. # only consider the target masks
            ...
            if i == len(self.train_loader) - 1 or self.terminate:
                ...
                if 'targets' in batch:
                    images['targets'] = [target.numpy() for target in batch['targets']]
    ...
    def _handle_lr(self) -> None:
        ...
        if last_lr > curr_lr < next_lr:
            ...
            # self._save_model(suffix=f'_minlr_step{self.step}') ## save model each lr cycle
    ...
    def _validate(self) -> Tuple[Dict[str, float], Dict[str, np.ndarray]]:
        ...
        for i, batch in batch_iter:
            ...
            dinp = inp.to(self.device, non_blocking=True)
            if targets is not None:
                dtargets = [target.to(self.device, non_blocking=True) for target in targets]
            else:
                dtargets = None
            dtarget_class = target_class.to(self.device, non_blocking=True) if target_class is not None else None
            with amp.autocast(self.device.type, enabled=self.mixed_precision):
                dout = self.model(dinp)
                if dtargets is None:  # Use self-supervised unary loss function
                    val_loss.append(self.ss_criterion(dout).item())
                elif dtarget_class is not None:
                    val_loss.append(self.criterion(dout, dtargets, dtarget_class).item())
                else:
                    val_loss.append(self.criterion(dout, dtargets).item())
            out = dout.detach().cpu()
            outs.append(out)
        ...
        images = {
            'inp': inp.numpy(),
            'out': out.numpy(),
            'targets': None if targets is None else [target.numpy() for target in targets],
        ...
        for name, evaluator in self.valid_metrics.items():
            mvals = [evaluator(target[0], out) for target, out in zip(all_targets, outs)]
...
#       extra dict whose contents will be added to the state_dict
    def _save_model(
        ...
        info = {
            ...
            'elektronn3.__version__': my_elektronn3.__version__,
